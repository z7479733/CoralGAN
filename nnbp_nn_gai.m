function nn = nnbp_nn_gai(nn_sou,nn_tar,nn)
    % 网络反向传播，返回更新后的权重 
   n = nn_sou.n+1;
    ...
    for i = (n-1) : -1 : 2 
        ...
        % Backpropagate first derivatives
         if i == n-1
            % in this case in d{n} there is not the bias term to be removed 在这种情况下，在d {n}中没有要去除的偏差项
            d_sou{i} = d_sou{i+1}.* d_act_sou;
            d_tar{i} = d_tar{i+1}.* d_act_tar; % 此处梯度不加负号
            % d_tar{i} = (d_tar{i + 1} * nn.W{i} + sparsityError) .* d_act;   
        else   % in this case in d{i} the bias term has to be removed 在这种情况下，必须在d {i}中删除偏差项
